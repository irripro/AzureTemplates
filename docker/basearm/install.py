...
def get_install_dir():
    ...
    while not install_dir:
        ...
        install_dir = os.path.realpath(os.path.expanduser(DEFAULT_INSTALL_DIR))
        if ' ' in install_dir:
...
def get_exec_dir():
    ...
    while not exec_dir:
        ...
        exec_dir = os.path.realpath(os.path.expanduser(DEFAULT_EXEC_DIR))
        if ' ' in exec_dir:
...
def get_rc_file_path():
    ...
    rc_file = rc_file or default_rc_file
    if rc_file:
